...
// $date = filter_input( INPUT_GET, 'データ' );
...
try
{
	...
	// $stmt = $db->query("SELECT * FROM comment"); //コメントテーブル引っ張ってくる
	// $comment = $stmt->fetchAll(PDO::FETCH_ASSOC);
	// $stmt = $db->query("SELECT * FROM commentreply"); // 返信テーブルを引っ張ってくる
	// $commetreply = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// 	$rensyu_id = $practice_id[$index] + 1;
// 	$stmt = $db->query("SELECT view_name,message,post_date FROM message ORDER BY post_date DESC");
...
console.log(c);
	...
	<head>
	   ...
	   <div class="header">
		   <div class="total">
			   <!--<div id="percent_id" class="total_child"></div>-->
	...
	<body>
	   ...
	   <div class="tab-content" id="myTabContent">
		...
		<!--<video width="300" height="200" autoplay muted playsinline src="画面収録 2020-07-17 0.55.48.mov" controls></video> -->
		...
		<!--<iframe width="560" height="315" src="https://www.youtube.com/embed/UFQEttrn6CQ" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>-->
				   </div>
				   <div class="result">
		...
		try
		{
			...
			$stmt = $db->query("SELECT view_name,message,post_date FROM message WHERE practice_id =" .$d["id"]."  ORDER BY post_date DESC");// $d[id]
			$message_array = $stmt->fetchAll(PDO::FETCH_ASSOC);
