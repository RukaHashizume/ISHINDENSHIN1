...
// if(!empty($teammokuhyouti['id'])){
	...
	if(empty($team_mokuhyouti[0])){
  	...
  	$stmt01=$db->query("SELECT pasento FROM demo_mokuhyoutasseiritu WHERE user_id=1 AND date=date");
...
<div class="all">
    <div class="ue1">
		...
		<p>目標速度：<?php if(!empty($teammokuhyou_speed)) echo $teammokuhyou_speed;?>m/s</p>
		<p>目標の高さ：<?php if(!empty($teammokuhyou_top)) echo $teammokuhyou_top;?>cm</p>   
    </div>
        
    <div class="ue2">
		<p>今日のチーム評価</p>
        <p>チーム全体の点数：<?php if(!empty($total)) echo $total;?></p>
		<p>平均速度：<?php if(!empty($ave_speed)) echo $ave_speed;?>m/s</p>
		<p>高さの平均：<?php if(!empty($ave_top)) echo $ave_top;?>cm</p>
		<p>命中ポイントの平均：<?php if(!empty($ave_meichu)) echo $ave_meichu;?></p>
    </div>
    <div class="sita1">
        <hr>
        <p>次回のチーム目標値</p>
        <p>次回の目標速度:<?php if(!empty($next_teammokuhyouspeed)) echo $next_teammokuhyouspeed;?>m/s</p>
        <p>次回の目標の高さ:<?php if(!empty($next_teammokuhyoutop)) echo $next_teammokuhyoutop;?>cm</p>
                <hr><p><br>チーム全体の目標値・サーブポイントの推移</p>
                ...
                target1=JSON.parse('<?php  if(!empty($target_json1)) echo $target_json1; ?>');
                ...
                percent1=JSON.parse('<?php if(!empty($percent_json1)) echo $percent_json1; ?>');
